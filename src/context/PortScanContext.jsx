    ...
    try {
     ...
     const resp = await axios.get(url, { timeout: 180000 });
      ...
      if (errorMessage.includes("nmap not installed")) {
        setError(
          "nmap is not installed on the server. Please install nmap to use port scanning."
        );
      }
    } finally {
      setLoading(false);
...
const resp = await axios.post(
  ...
  { timeout: 60000 }
      ...
      console.error(
        "correlateServices error",
        err.response?.data || err.message
      );
      setError(
        err.response?.data?.message || err.message || "Correlation failed"
      );
    } finally {
      setLoading(false);
    }
